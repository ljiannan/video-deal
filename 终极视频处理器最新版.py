...
# --- FFmpeg 路径配置 ---
FFMPEG_PATH = r"C:\新建文件夹\ffmpeg-master-latest-win64-gpl\ffmpeg-master-latest-win64-gpl\bin\ffmpeg.exe"
FFPROBE_PATH = r"C:\新建文件夹\ffmpeg-master-latest-win64-gpl\ffmpeg-master-latest-win64-gpl\bin\ffprobe.exe"
# --- 数据库记录字段配置 ---
# !!! 重要：每次视频处理成功后，以下11个字段信息会插入Video_Editing表 !!!
OPERATOR_NAME = "L"                   # 操作人员姓名/用户名 (必填)
COMPUTER_NAME = "D69"                # 电脑名称 (留空则自动获取当前电脑名)
...
# --- 输入输出路径配置 ---
INPUT_DIR = r"Z:\a项目\航拍特写\李建楠\9.24  2kh"
OUTPUT_DIR = r"D:\9.24干完"
...
# --- 切头尾配置 (仅切头尾模式) ---
CUT_HEAD_SECONDS = 65     # 切掉开头秒数
CUT_TAIL_SECONDS = 65      # 切掉结尾秒数
...
MAX_FILTER_WORKERS = 8                # 最大过滤并行数（防死锁优化）
...
class DistributedTaskManager:
    ...
    def claim_video_task(self, video_path: str) -> bool:
        ...
        try:
            ...
            # 🔍 额外检查输出文件是否已存在（防止数据库记录丢失）
            try:
                base_name = os.path.splitext(video_name)[0]
                processed_file = os.path.join(OUTPUT_DIR, f"{base_name}.mp4")
                if os.path.exists(processed_file):
                    file_size = os.path.getsize(processed_file)
                    if file_size > 1024:  # 大于1KB认为是有效文件
                        logger.warning(f"🚨 发现输出文件但数据库无记录: {processed_file} ({file_size:,} bytes)")
                        # 虽然数据库没记录，但文件已存在，应该跳过处理
                        return False
            except Exception as e:
                logger.debug(f"检查输出文件时出错: {e}")
            
            # 2. 创建处理锁表（如果不存在）
            cursor.execute(f"""
                CREATE TABLE IF NOT EXISTS `{TABLE_NAME}_processing_locks` (
            ...
            # 3. 清理过期的处理锁（超过2小时）
            cursor.execute(f"""
                DELETE FROM `{TABLE_NAME}_processing_locks` 
                WHERE heartbeat_time < DATE_SUB(NOW(), INTERVAL 2 HOUR)
            """)
            
            # 4. 检查是否有其他电脑正在处理
            cursor.execute(f"""
                SELECT computer_name, claimed_time FROM `{TABLE_NAME}_processing_locks`
                WHERE video_name = %s
            """, (video_name,))
            
            processing_record = cursor.fetchone()
            if processing_record:
                other_computer = processing_record[0]
                claimed_time = processing_record[1]
                logger.info(f"⏳ 视频正被 {other_computer} 处理中（开始于 {claimed_time}）: {video_name}")
                return False
            
            # 5. 尝试获取处理锁
            try:
                cursor.execute(f"""
                    INSERT INTO `{TABLE_NAME}_processing_locks` 
                    (video_name, computer_name, computer_ip)
                    VALUES (%s, %s, %s)
                """, (video_name, self.computer_name, self.db_manager.computer_ip))
                
                logger.debug(f"🔐 获取分布式处理锁成功: {video_name}")
                return True
                
            except Exception as insert_e:
                # 可能是重复键错误，说明其他电脑抢先获取了锁
                logger.info(f"🔒 其他电脑抢先获取处理锁: {video_name}")
                return False
    ...
    def release_video_task(self, video_path: str):
        ...
        try:
            ...
            cursor.execute(f"""
                DELETE FROM `{TABLE_NAME}_processing_locks` 
                WHERE video_name = %s AND computer_name = %s
            """, (video_name, self.computer_name))
            
            deleted_count = cursor.rowcount
            if deleted_count > 0:
                logger.debug(f"🔓 释放分布式处理锁: {video_name}")
            else:
                logger.debug(f"🔓 处理锁已不存在: {video_name}")
                
        except Exception as e:
            logger.warning(f"⚠️ 释放分布式锁失败: {e}")
    
    def update_processing_heartbeat(self, video_path: str):
        """更新处理心跳，防止锁被清理"""
        if not ENABLE_DISTRIBUTED_PROCESSING or not self.db_manager.is_enabled:
            return
        ...
        try:
            ...
            cursor.execute(f"""
                UPDATE `{TABLE_NAME}_processing_locks` 
                SET heartbeat_time = CURRENT_TIMESTAMP
                WHERE video_name = %s AND computer_name = %s
            """, (video_name, self.computer_name))
            
        except Exception as e:
            logger.warning(f"⚠️ 更新处理心跳失败: {e}")
...
class DatabaseManager:
    ...
    def execute_with_retry(self, query: str, params: tuple = None, max_retries: int = 3, timeout: int = 30):
        """执行SQL语句（带重试机制和超时控制）"""
        if not self.is_enabled:
            return None
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"⏰ SQL执行超时 ({timeout}s) (尝试 {attempt + 1}/{max_retries}): {query[:50]}...")
                # 超时时重新创建连接
                try:
                    self.connection_pool = self._create_robust_connection()
                except Exception as conn_e:
                    logger.error(f"❌ 超时后重新连接失败: {conn_e}")
                
                if attempt == max_retries - 1:
                    logger.error(f"❌ SQL执行最终超时: {query[:100]}...")
                    return None
                ...
                # 连接错误时重新创建连接
                if "connection" in str(e).lower() or "lost" in str(e).lower() or "timeout" in str(e).lower():
                    try:
                        self.connection_pool = self._create_robust_connection()
                    except Exception as conn_e:
                ...
                time.sleep(min(2 * (attempt + 1), 10))  # 递增等待时间，最大10秒
    ...
    def _calculate_video_sha256(self, video_path: str, quick_mode: bool = True) -> str:
        """计算视频文件SHA256 - 防死锁优化版本"""
        try:
            if not os.path.exists(video_path):
                return ""
            
            # 🚨 大文件检查：超过2GB的文件强制使用快速模式，避免长时间计算
            file_size = os.path.getsize(video_path)
            if file_size > 2 * 1024 * 1024 * 1024:  # 2GB
                quick_mode = True
                logger.debug(f"📏 大文件检测({file_size // 1024 // 1024}MB)，使用快速哈希模式")
            ...
            if quick_mode:
                # 快速模式：采样文件头、中间、尾部
                sample_size = min(512 * 1024, file_size // 20)  # 512KB或文件的1/20，减少采样大小
                ...
                # 添加文件大小和修改时间作为补充信息，提高唯一性
                file_stat = os.stat(video_path)
                hash_sha256.update(str(file_size).encode())
                hash_sha256.update(str(int(file_stat.st_mtime)).encode())
                
            else:
                # 完整模式：但增加进度控制，防止超大文件卡死
                ...
                with open(video_path, 'rb') as f:
                    while chunk := f.read(chunk_size):
                        ...
                        # 🚨 防止超时：处理超过100MB后中断（对于过滤阶段足够了）
                        if processed_size > 100 * 1024 * 1024:
                            logger.debug(f"⏰ 哈希计算达到限制(100MB)，提前结束: {os.path.basename(video_path)}")
                            # 添加已处理的大小作为补充信息
                            hash_sha256.update(str(processed_size).encode())
                            break
            
            hash_result = hash_sha256.hexdigest()
            logger.debug(f"🔍 哈希计算完成: {os.path.basename(video_path)} -> {hash_result[:16]}...")
            return hash_result
            
        except Exception as e:
            logger.error(f"❌ 计算SHA256失败 {os.path.basename(video_path)}: {e}")
            return ""
...
class SmartCacheManager:
    ...
    def cleanup_video_cache(self, video_path: str):
        """清理特定视频的缓存文件"""
        try:
            video_name = os.path.basename(video_path)
            freed_mb = 0
            ...
            # 清理智能缓存文件
            if video_path in self.cache_records:
                cache_record = self.cache_records[video_path]
                cache_path = cache_record.get('cache_path')
                ...
                if cache_path and os.path.exists(cache_path):
                    try:
                        ...
                        logger.info(f"🗑️ 删除智能缓存文件: {video_name} ({file_size:,} bytes)")
                        ...
                        # 从完成记录中移除
                        if video_path in self.completed_cache_videos:
                            self.completed_cache_videos.remove(video_path)
                            self._save_cache_completion_record()
                        
                    except Exception as e:
                        logger.warning(f"⚠️ 删除缓存文件失败: {cache_path} - {e}")
            
            return freed_mb
            
        except Exception as e:
            logger.warning(f"⚠️ 清理视频缓存异常: {os.path.basename(video_path)} - {e}")
            return 0
...
class ProgressManager:
    ...
    def atomic_check_and_mark_processing(self, video_path: str, output_dir: str = None) -> Tuple[bool, str]:
        """原子性检查并标记为处理中，防止重复处理
        ...
        # 原子性检查：在同一个操作中检查和标记
        if self.is_completed(video_path):
            return False, "已完成"
        ...
        if output_dir:
            ...
            for output_file in possible_outputs:
                if os.path.exists(output_file):
                    try:
                        ...
                        if file_size > 1024:  # 大于1KB认为是有效文件
                            logger.debug(f"🔍 发现已存在的输出文件: {output_file} ({file_size:,} bytes)")
                            return False, f"输出文件已存在: {os.path.basename(output_file)}"
                    except Exception:
                        pass
            ...
            try:
                ...
                # 检查是否有已处理文件
                similar_files = [f for f in output_files if f == f"{base_name}.mp4"]
                
                if similar_files:
                    logger.info(f"🔍 发现相似的已处理文件: {similar_files}")
                    # 检查最大的文件（通常是完整的）
                    largest_file = None
                    largest_size = 0
                    for sim_file in similar_files:
                        full_path = os.path.join(output_dir, sim_file)
                        try:
                            size = os.path.getsize(full_path)
                            if size > largest_size:
                                largest_size = size
                                largest_file = sim_file
                        except:
                            continue
                    
                    if largest_file and largest_size > 1024:
                        return False, f"相似文件已存在: {largest_file} ({largest_size:,} bytes)"
                        
            except Exception as e:
                logger.debug(f"模糊匹配检查失败: {e}")
...
class VideoProcessor:
    ...
    def __init__(self, hardware_info: Dict[str, Any], progress_manager: ProgressManager):
        ...
        # 🔧 修复：使用指定的缓存目录，而不是在运行目录下创建
        self.temp_dir = Path(CACHE_DIR) / "temp_processing"
        self.temp_dir.mkdir(parents=True, exist_ok=True)
        logger.info(f"📁 临时处理目录: {self.temp_dir}")
    
    def cleanup_video_cache_files(self, video_path: str):
        """清理特定视频的所有缓存文件（临时处理文件）"""
        try:
            video_name = os.path.basename(video_path)
            base_name = os.path.splitext(video_name)[0]
            ...
            # 清理临时处理目录中的相关文件
            cleanup_patterns = [
                f"{base_name}_temp_cropped.mp4",
                f"{base_name}_temp_trimmed.mp4", 
                f"{base_name}_temp_*.mp4",
                f"{base_name}.tmp",
                f"{base_name}_processing.*"
            ]
                ...
                try:
                    ...
                    for temp_file in temp_files:
                        if temp_file.exists():
                            ...
                            logger.debug(f"🗑️ 删除临时文件: {temp_file.name} ({file_size:,} bytes)")
                except Exception as e:
                    logger.warning(f"⚠️ 清理临时文件失败 {pattern}: {e}")
            
            if cleaned_files:
                logger.info(f"🧹 清理完成: {video_name} -> 删除 {len(cleaned_files)} 个临时文件，释放 {total_freed_mb:.1f}MB")
            else:
                logger.debug(f"🧹 无需清理: {video_name} (无临时文件)")
                
        except Exception as e:
            logger.warning(f"⚠️ 清理视频缓存文件异常: {os.path.basename(video_path)} - {e}")
    ...
    def get_video_info(self, video_path: str, timeout: int = 20) -> Dict[str, Any]:
        """获取视频信息（防死锁优化版）"""
        try:
            # 🚨 快速检查文件可读性
            if not os.path.exists(video_path):
                logger.warning(f"⚠️ 文件不存在: {os.path.basename(video_path)}")
                return {}
            
            file_size = os.path.getsize(video_path)
            if file_size == 0:
                logger.warning(f"⚠️ 文件为空: {os.path.basename(video_path)}")
                return {}
            
            # 使用更简单的ffprobe命令，只获取基本信息
            probe_cmd = [
            ...
            result = subprocess.run(
                ...
                encoding='utf-8', 
                errors='ignore',
                stdin=subprocess.DEVNULL  # 防止等待stdin输入
            )
            
            if result.returncode != 0:
                stderr = result.stderr.strip()
                if stderr:
                    logger.warning(f"⚠️ ffprobe失败: {os.path.basename(video_path)} - {stderr[:100]}")
                else:
                    logger.warning(f"⚠️ ffprobe失败: {os.path.basename(video_path)} - 未知错误")
                return {'file_size': file_size, 'error': 'probe_failed'}
            
            if not result.stdout.strip():
                logger.warning(f"⚠️ ffprobe无输出: {os.path.basename(video_path)}")
                return {'file_size': file_size, 'error': 'no_output'}
            
            try:
                data = json.loads(result.stdout)
            except json.JSONDecodeError as e:
                logger.warning(f"⚠️ JSON解析失败: {os.path.basename(video_path)} - {e}")
                return {'file_size': file_size, 'error': 'json_decode_failed'}
            
            # 查找视频流
            streams = data.get('streams', [])
            if not streams:
                logger.warning(f"⚠️ 未找到视频流: {os.path.basename(video_path)}")
                return {'file_size': file_size, 'error': 'no_video_stream'}
            
            video_stream = streams[0]  # 取第一个视频流
            
            # 🚨 安全的类型转换，避免eval导致的安全问题
            def safe_fps_parse(fps_str):
                try:
                    if '/' in str(fps_str):
                        num, den = map(float, str(fps_str).split('/', 1))
            ...
            info = {
                ...
                'duration': safe_float(video_stream.get('duration'), 0),
                'fps': safe_fps_parse(video_stream.get('r_frame_rate', '25/1')),
            ...
            format_duration = safe_float(format_info.get('duration'))
            if format_duration > 0:
                info['duration'] = format_duration
            
            # 基本有效性检查
            if info['width'] == 0 or info['height'] == 0:
                logger.warning(f"⚠️ 无效分辨率: {os.path.basename(video_path)} - {info['width']}x{info['height']}")
                info['error'] = 'invalid_resolution'
            
            logger.debug(f"📹 视频信息: {os.path.basename(video_path)} - {info['width']}x{info['height']}, {info['duration']:.1f}s")
            return info
            
        except subprocess.TimeoutExpired:
            logger.warning(f"⏰ ffprobe超时: {os.path.basename(video_path)} ({timeout}s)")
            return {'file_size': os.path.getsize(video_path) if os.path.exists(video_path) else 0, 'error': 'timeout'}
        except Exception as e:
            logger.error(f"❌ 获取视频信息失败: {os.path.basename(video_path)} - {e}")
            return {'file_size': os.path.getsize(video_path) if os.path.exists(video_path) else 0, 'error': f'exception: {e}'}
    
    def should_skip_low_resolution(self, video_path: str) -> Tuple[bool, str]:
        """检查是否应跳过低分辨率视频（防死锁优化版）"""
        if not SKIP_LOW_RESOLUTION:
            return False, ""
        
        try:
            # 使用较短的超时时间进行快速检查
            video_info = self.get_video_info(video_path, timeout=15)
            
            # 检查是否有错误
            if 'error' in video_info:
                error_type = video_info['error']
                if error_type in ['timeout', 'probe_failed', 'no_video_stream']:
                    logger.warning(f"⚠️ 无法检测分辨率，跳过: {os.path.basename(video_path)} - {error_type}")
                    return True, f"检测失败({error_type})"
                elif error_type == 'invalid_resolution':
                    return True, "无效分辨率"
                else:
                    # 其他错误不跳过，让后续处理决定
                    return False, f"检测异常: {error_type}"
            
            width = video_info.get('width', 0)
            height = video_info.get('height', 0)
            
            if width == 0 or height == 0:
                return False, "无法获取有效分辨率"
            
            if width < MIN_RESOLUTION_WIDTH:
                reason = f"分辨率({width}x{height})低于最小要求({MIN_RESOLUTION_WIDTH}px宽度)"
                logger.info(f"📱 检测到低分辨率: {os.path.basename(video_path)} - {reason}")
                return True, reason
            
            logger.debug(f"✅ 分辨率检查通过: {os.path.basename(video_path)} - {width}x{height}")
            return False, f"分辨率合格({width}x{height})"
            
        except Exception as e:
            logger.warning(f"⚠️ 分辨率检查异常: {os.path.basename(video_path)} - {e}")
            # 异常时不跳过，让后续处理决定
...
class UnifiedVideoProcessor:
    ...
    def __init__(self):
        ...
        # 🔒 线程级文件锁定机制，防止重复处理
        self.file_processing_locks = {}
        self.file_locks_lock = threading.RLock()  # 保护文件锁字典的锁
    ...
    def acquire_file_processing_lock(self, file_path: str) -> Tuple[bool, Optional[threading.Lock]]:
        """获取文件处理锁，防止重复处理同一文件
        ...
        with self.file_locks_lock:
            if file_key not in self.file_processing_locks:
                self.file_processing_locks[file_key] = threading.Lock()
            file_lock = self.file_processing_locks[file_key]
        
        # 尝试获取锁，非阻塞
        acquired = file_lock.acquire(blocking=False)
        if acquired:
            logger.debug(f"🔒 获取文件处理锁成功: {os.path.basename(file_path)}")
            return True, file_lock
        else:
            logger.info(f"⏭️ 文件正在被其他线程处理，跳过: {os.path.basename(file_path)}")
            return False, None
    
    def release_file_processing_lock(self, file_path: str, file_lock: threading.Lock):
        """释放文件处理锁"""
        try:
            file_lock.release()
            logger.debug(f"🔓 释放文件处理锁: {os.path.basename(file_path)}")
        except Exception as e:
            logger.warning(f"⚠️ 释放文件锁异常: {e}")
    
    def get_file_processing_locks_status(self) -> Dict[str, int]:
        """获取文件锁状态信息"""
        with self.file_locks_lock:
            total_locks = len(self.file_processing_locks)
            active_locks = sum(1 for lock in self.file_processing_locks.values() if lock.locked())
            return {
                'total_locks': total_locks,
                'active_locks': active_locks
            }
    
    def cleanup_duplicate_processed_files(self, output_dir: str):
        """清理重复的输出文件，只保留最大的"""
        if not os.path.exists(output_dir):
            return
        
        try:
            logger.info("🧹 开始清理重复的输出文件...")
            
            # 获取所有输出文件，按原始文件名分组
            processed_files = {}  # original_base_name -> [file_paths]
            ...
            for filename in os.listdir(output_dir):
                if filename.endswith('.mp4'):
                    ...
                    # 去掉可能的处理后缀
                    for suffix in ['_processed', '_cropped', '_trimmed', '_temp_cropped']:
                    ...
                    processed_files[base_name].append(os.path.join(output_dir, filename))
            
            # 检查并清理重复文件
            total_cleaned = 0
            total_freed_mb = 0
            
            for base_name, file_list in processed_files.items():
                if len(file_list) > 1:
                    logger.warning(f"🔍 发现重复的输出文件: {base_name} -> {len(file_list)} 个文件")
                    ...
                    for file_path in file_list:
                        if file_path != largest_file:
                            try:
                                ...
                                os.remove(file_path)
                                total_cleaned += 1
                                total_freed_mb += size / (1024 * 1024)
                                logger.info(f"🗑️ 删除重复文件: {os.path.basename(file_path)} ({size:,} bytes)")
                            except Exception as e:
                                logger.warning(f"⚠️ 删除文件失败 {file_path}: {e}")
                    
                    if largest_file:
                        logger.info(f"✅ 保留最大文件: {os.path.basename(largest_file)} ({largest_size:,} bytes)")
            
            if total_cleaned > 0:
                logger.info(f"🧹 清理完成: 删除了 {total_cleaned} 个重复文件，释放 {total_freed_mb:.1f}MB 空间")
            else:
                logger.info("✅ 未发现重复的输出文件")
                
        except Exception as e:
            logger.error(f"清理重复输出文件时出错: {e}")

    def _cleanup_all_video_cache_files(self, video_path: str):
        """清理特定视频的所有缓存文件（智能缓存 + 临时处理文件）"""
        try:
            video_name = os.path.basename(video_path)
            total_freed_mb = 0
            ...
            if self.cache_manager and self.cache_manager.is_enabled:
                freed_mb = self.cache_manager.cleanup_video_cache(video_path)
                total_freed_mb += freed_mb
            
            # 2. 清理临时处理文件
            self.video_processor.cleanup_video_cache_files(video_path)
            
            if total_freed_mb > 0:
                logger.info(f"🧹 视频缓存清理完成: {video_name} -> 总共释放 {total_freed_mb:.1f}MB")
            else:
                logger.debug(f"🧹 视频缓存清理完成: {video_name} (无缓存文件需要清理)")
                
        except Exception as e:
            logger.warning(f"⚠️ 视频缓存清理异常: {os.path.basename(video_path)} - {e}")
    ...
    def graceful_shutdown(self):
        ...
        try:
            # 显示文件锁状态
            lock_status = self.get_file_processing_locks_status()
            logger.info(f"🔒 文件锁状态: 总锁数={lock_status['total_locks']}, 活跃锁数={lock_status['active_locks']}")
    ...
    def filter_videos(self, video_files: List[str]) -> List[str]:
        ...
        logger.info(f"🔍 开始过滤 {total_files} 个视频文件...")
        
        # 使用进度条显示过滤进度
        with tqdm(total=total_files, desc="🔍 过滤视频", unit="file", 
                 bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as pbar:
            
            # 使用线程池并行过滤，限制并发数避免资源耗尽
            max_filter_workers = min(MAX_FILTER_WORKERS, total_files, self.hardware_info['max_parallel'] // 2)
            logger.info(f"⚡ 使用 {max_filter_workers} 个线程并行过滤")
            
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_filter_workers) as executor:
                # 提交所有过滤任务
                future_to_video = {
                    executor.submit(self._filter_single_video_safe, video_path, pbar): video_path
                ...
                for future in concurrent.futures.as_completed(future_to_video):
                    ...
                    try:
                        # 设置过滤超时，防止单个文件卡死整个过程
                        result = future.result(timeout=60)  # 每个文件最多60秒
                        ...
                        if result == "keep":
                            ...
                            error_count += 1
                            logger.warning(f"⚠️ 过滤异常，但继续处理: {os.path.basename(video_path)}")
                        # "completed" 情况不计数，已在线程内处理
                        
                    except concurrent.futures.TimeoutError:
                        logger.error(f"⏰ 过滤超时，跳过: {os.path.basename(video_path)}")
                        error_count += 1
                        pbar.update(1)
                    except Exception as e:
                        logger.error(f"❌ 过滤任务异常: {os.path.basename(video_path)} - {e}")
                        error_count += 1
                        pbar.update(1)
        
        # 过滤完成统计
        total_processed = len(filtered_files) + skipped_count + error_count
        success_rate = (len(filtered_files) / total_processed * 100) if total_processed > 0 else 0
        
        logger.info(f"✅ 过滤完成统计:")
        logger.info(f"   📋 待处理: {len(filtered_files)} 个 ({success_rate:.1f}%)")
        logger.info(f"   ⏭️ 跳过: {skipped_count} 个")
        logger.info(f"   ❌ 异常: {error_count} 个")
        
        return filtered_files
    
    def _filter_single_video_safe(self, video_path: str, pbar: tqdm = None) -> str:
        """安全地过滤单个视频文件（防死锁版本）
        ...
        try:
            # 1. 检查文件是否存在（快速检查）
            if not os.path.exists(video_path):
                logger.warning(f"⚠️ 文件不存在: {video_name}")
                if pbar:
                    pbar.update(1)
                return "error"
            ...
            # 3. 带超时的低分辨率检查
            try:
                should_skip, reason = self._check_resolution_with_timeout(video_path, timeout=30)
                if should_skip:
                    logger.info(f"📱 跳过低分辨率: {video_name} - {reason}")
                    
                    # 异步移动文件，不阻塞主流程
                    if SKIP_VIDEOS_MOVE_DIR and os.path.exists(SKIP_VIDEOS_MOVE_DIR):
                        threading.Thread(
                            target=self._move_file_async,
                            args=(video_path, SKIP_VIDEOS_MOVE_DIR, video_name),
                            daemon=True
                        ).start()
                ...
                logger.warning(f"⚠️ 分辨率检查失败: {video_name} - {e}")
                # 检查失败不跳过，继续处理
            
            # 4. 带超时的数据库查重检查
            if self.db_manager.is_enabled:
                try:
                    logger.debug(f"🔍 数据库查重: {video_name}")
                    is_duplicate, message = self._enhanced_duplicate_check_with_timeout(video_path, timeout=45)
                    
                    if is_duplicate:
                        logger.info(f"🔄 发现重复，跳过: {video_name} - {message}")
                        if pbar:
                            pbar.update(1)
                        return "skip"
                    else:
                        logger.debug(f"✅ 查重通过: {video_name}")
                except Exception as e:
                    logger.warning(f"⚠️ 数据库查重失败: {video_name} - {e}")
                    # 查重失败不跳过，继续处理
            ...
            logger.error(f"❌ 过滤单个文件异常: {video_name} - {e}")
            if pbar:
                pbar.update(1)
            return "error"
    
    def _check_resolution_with_timeout(self, video_path: str, timeout: int = 30) -> Tuple[bool, str]:
        """带超时的分辨率检查"""
        if not SKIP_LOW_RESOLUTION:
            return False, ""
        ...
        try:
            # 使用future实现超时控制
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(self.video_processor.should_skip_low_resolution, video_path)
                return future.result(timeout=timeout)
                
        except concurrent.futures.TimeoutError:
            logger.warning(f"⏰ 分辨率检查超时: {os.path.basename(video_path)}")
            return False, "检查超时"
        except Exception as e:
            logger.warning(f"⚠️ 分辨率检查异常: {e}")
            return False, f"检查异常: {e}"
    
    def _enhanced_duplicate_check_with_timeout(self, video_path: str, timeout: int = 45) -> Tuple[bool, str]:
        """带超时的增强查重检查"""
        try:
            # 使用future实现超时控制
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(self.db_manager.enhanced_duplicate_check, video_path)
                return future.result(timeout=timeout)
                
        except concurrent.futures.TimeoutError:
            logger.warning(f"⏰ 数据库查重超时: {os.path.basename(video_path)}")
            return False, "查重超时"
        except Exception as e:
            logger.warning(f"⚠️ 数据库查重异常: {e}")
            return False, f"查重异常: {e}"
    
    def _move_file_async(self, source_path: str, target_dir: str, file_name: str):
        """异步移动文件，避免阻塞主流程"""
        try:
            ...
            logger.info(f"📁 异步移动完成: {dest_path}")
            
        except Exception as e:
            logger.warning(f"⚠️ 异步移动文件失败: {file_name} - {e}")
    ...
    def process_video_file(self, video_path: str, roi: Optional[Tuple[int, int, int, int]] = None) -> bool:
        ...
        # 🔒 关键修复：获取线程级文件锁，防止同一进程内重复处理
        lock_acquired, file_lock = self.acquire_file_processing_lock(video_path)
        if not lock_acquired:
            logger.info(f"⏭️ 文件正在被其他线程处理，跳过: {video_name}")
            lock_status = self.get_file_processing_locks_status()
            logger.debug(f"当前锁状态: 总锁数={lock_status['total_locks']}, 活跃锁数={lock_status['active_locks']}")
            return False
        
        try:
            # 1. 原子性检查并标记为处理中（在文件锁保护下）
            can_process, reason = self.progress_manager.atomic_check_and_mark_processing(video_path, OUTPUT_DIR)
            if not can_process:
                logger.info(f"⏭️ 跳过处理: {video_name} - {reason}")
                return False  # 返回False表示跳过，不是失败
            
            # 2. 分布式任务声明
            if not self.task_manager.claim_video_task(video_path):
                logger.info(f"🔒 视频已被其他电脑处理: {video_name}")
                # 移除本地处理中标记，因为其他电脑在处理
                self.progress_manager.progress_data['processing'] = [
                    p for p in self.progress_manager.progress_data['processing'] if p != video_name
                ]
            ...
            # 5. 智能缓存处理：使用缓存路径进行处理，但数据库记录使用原始路径
            ...
            # 9. 更新分布式处理心跳
            self.task_manager.update_processing_heartbeat(original_video_path)
            ...
            if self.mode == ProcessingMode.CROP:
                ...
                success = self.video_processor.process_video_crop(actual_processing_path, final_output_path, roi, self.roi_selector)
            
            elif self.mode == ProcessingMode.TRIM:
                success = self.video_processor.process_video_trim(
                    actual_processing_path, final_output_path, CUT_HEAD_SECONDS, CUT_TAIL_SECONDS, SEGMENT_DURATION
                )
                ...
                if os.path.exists(final_output_path):
                    try:
                        ...
                        if existing_size > 1024:
                            logger.warning(f"⚠️ 输出文件在处理期间被创建: {final_output_path} ({existing_size:,} bytes)")
                            return False  # 避免覆盖已存在的文件
                    except Exception as e:
                        logger.debug(f"检查输出文件大小时出错: {e}")
                ...
                if crop_success:
                    ...
                    if os.path.exists(final_output_path):
                        try:
                            existing_size = os.path.getsize(final_output_path)
                            if existing_size > 1024:
                                logger.warning(f"⚠️ 输出文件在裁剪后被创建: {final_output_path} ({existing_size:,} bytes)")
                                return False  # 避免覆盖已存在的文件
                        except Exception as e:
                            logger.debug(f"最终检查输出文件大小时出错: {e}")
                    
                    # 再切头尾到最终输出文件
                    logger.info(f"🎬 开始生成最终输出文件: {final_output_path}")
                    success = self.video_processor.process_video_trim(
                        str(temp_cropped), final_output_path, CUT_HEAD_SECONDS, CUT_TAIL_SECONDS, SEGMENT_DURATION
                    )
            ...
            if success and os.path.exists(final_output_path):
                ...
                if output_size > 1024:  # 文件必须大于1KB
                    logger.info(f"✅ 输出文件验证成功: {os.path.basename(final_output_path)} ({output_size:,} bytes)")
                else:
                    logger.error(f"❌ 输出文件太小: {os.path.basename(final_output_path)} ({output_size} bytes)")
                    success = False
                    # 🧹 验证失败时清理临时文件，避免磁盘占用
                    self._cleanup_all_video_cache_files(original_video_path)
            elif success:
                logger.error(f"❌ 输出文件不存在: {final_output_path}")
                success = False
                # 🧹 验证失败时清理临时文件，避免磁盘占用
                self._cleanup_all_video_cache_files(original_video_path)
            ...
            if success:
                ...
                # 标记为完成
                self.progress_manager.mark_completed(original_video_path, final_output_path, processing_time)
                ...
                # 🧹 处理失败时的清理已在验证阶段完成，这里只需处理其他情况
                if not (final_output_path and (not os.path.exists(final_output_path) or os.path.getsize(final_output_path) <= 1024)):
                    # 如果不是因为输出文件验证失败，才需要清理
                    self._cleanup_all_video_cache_files(original_video_path)
            ...
            # 🔓 关键修复：无论如何都要释放线程级文件锁
            try:
                if 'file_lock' in locals() and file_lock:
                    self.release_file_processing_lock(original_video_path, file_lock)
            except Exception as lock_e:
                logger.warning(f"⚠️ 释放文件锁时出现异常: {lock_e}")
    ...
    def run(self):
        ...
        try:
            ...
            # 🧹 清理重复的_processed文件
            print("🧹 检查并清理重复的输出文件...")
            self.cleanup_duplicate_processed_files(OUTPUT_DIR)
