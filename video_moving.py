...
class Config:
    """脚本的所有配置项"""
    # --- 路径配置 ---
    SOURCE_DIR: Path = Path(r"C:\douyu_400t")  # 源文件夹：存放待移动视频的文件夹
    DEST_DIR: Path = Path(r"Z:\a项目\douyu_400t")  # 目标文件夹：视频最终移动到的文件夹
    FFMPEG_PATH: Path = Path(r"C:\ffmpeg-master-latest-win64-gpl\bin\ffmpeg.exe")
    FFPROBE_PATH: Path = Path(r"C:\ffmpeg-master-latest-win64-gpl\bin\ffprobe.exe")  # ffprobe路径，通常与ffmpeg在一起
    LOG_FILE: Path = Path(r"C:\configuration\huya_log\huya_video_move_log.log")

    # --- 规则配置 ---
    VIDEO_EXTENSIONS: frozenset = frozenset({'.mp4', '.mov', '.avi', '.mkv', '.flv', '.wmv'})  # 允许的视频文件扩展名（不区分大小写）
    TRIGGER_SIZE_GB: float = 20.0  # 触发移动任务的源文件夹最小体积（单位：GB）

    # --- 时间窗口配置 (小时, 分钟) ---
    # 晚上8点
    TIME_WINDOW_START: tuple[int, int] = (9, 0)  # 在此时间之后，忽略文件夹大小限制
    # 上午9点
    TIME_WINDOW_END: tuple[int, int] = (9, 0)  # 在此时间之前，忽略文件夹大小限制
    ...
    # --- 性能配置 ---
    MAX_WORKERS: int = 4  # 并行处理文件的最大线程数，可根据CPU和磁盘性能调整
