...
class Config:
    # 视频保存路径
    SAVE_DIRECTORY = r"/Volumes/share-1/a项目/huya_400t"
    # MySQL数据库配置
    MYSQL_CONFIG = {
        'host': '192.168.10.70',
        'user': 'root',
        ...
        try:
            with requests.get(url, headers=headers, stream=True, timeout=(15, 60)) as resp:
                ...
                with tqdm(
                    ...
                    initial=downloaded_size, bar_format=bar_format, leave=False
                ) as pbar:
...
def process_task(db_manager: DatabaseManager, task: dict, resume: bool = False) -> bool:
    ...
    # 使用UUID生成唯一文件名，避免特殊字符和重名问题
    filename = task.get('filename') or f"{uuid.uuid4()}_{quality}.mp4"
...
if __name__ == "__main__":
    ...
    if not hostname:
        logger.error("启动失败：无法自动获取主机名，请检查网络配置。")
        sys.exit(1)
    logger.info(f"当前运行主机（自动获取）：{hostname}")
